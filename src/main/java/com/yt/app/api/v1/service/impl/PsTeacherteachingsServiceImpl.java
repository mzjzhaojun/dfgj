...
public class PsTeacherteachingsServiceImpl extends BaseServiceImpl<PsTeacherteachings, Long> implements PsTeacherteachingsService {
	...
	@Override
	@Transactional
	public Integer post(PsTeacherteachings t) {
	...
	@Override
	public IPage<PsTeacherteachings> list(RequestEntity<Object> requestEntity) {
		...
		List<PsTeacherteachings> list = mapper.list(param);
		return new PageBean<PsTeacherteachings>(param, list, count);
	}

	@Override
	public PsTeacherteachings get(Long id) {
		PsTeacherteachings t = mapper.get(id);
		return t;
	}
}