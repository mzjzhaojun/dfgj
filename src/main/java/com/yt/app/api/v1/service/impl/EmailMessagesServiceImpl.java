...
public class EmailMessagesServiceImpl extends BaseServiceImpl<EmailMessages, Long> implements EmailMessagesService {
	...
	@Override
	@Transactional
	public Integer post(EmailMessages t) {
	...
	@Override
	public IPage<EmailMessages> list(RequestEntity<Object> requestEntity) {
		...
		List<EmailMessages> list = mapper.list(param);
		return new PageBean<EmailMessages>(param, list, count);
	}

	@Override
	public EmailMessages get(Long id) {
		EmailMessages t = mapper.get(id);
		return t;
	}
}