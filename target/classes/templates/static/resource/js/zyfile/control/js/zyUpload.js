	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.init = function(){
				...
				this.funViewImgPreviewHtml(param.imgUrl);//回显图片
			};
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';//图片上传
		           ...
		           /* html += '			<div class="status_bar">';
				    ...
				    html += "							<img class='upload_image' src='../../../../resource/css/zyfile/control/images/add_img.png' onerror='src="+"\"../../../resource/css/zyfile/control/images/add_img.png\""+"'  style='width:expression(this.width > "+imgWidth+" ? "+imgWidth+"px : this.width)' />";
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			/**
			 * 功能： 回显数据中已存在的图片预览
			 * 参数: imgUrl 图片集字符串已逗号分割
			 * 返回: 预览的html
			 */
			this.funViewImgPreviewHtml = function(imgUrl){
				var html = "";
				if(imgUrl != undefined){
					var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
					...
					for(var i = 0 ;i<url_len;i++){
						delHtml = '<span class="file_del1" data-index="v_'+i+'" title="删除"></span>';
						html += '<div id="uploadList_v_'+ i +'" class="upload_append_list">';
						html += '	<div class="file_bar">';
						html += '		<div style="padding:5px;">';
						/*html += '			<p class="file_name">' + "22" + '</p>';*/
						html += delHtml;   // 删除按钮的html
						html += '		</div>';
						html += '	</div>';
						html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox">';
						html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
						html += '			<img id="uploadImage_v_'+i+'" class="upload_image" src="' + url[i] + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
						html += '		</div>';
						html += '	</a>';
						/*html += '	<p id="uploadProgress_'+i+'" class="file_progress"></p>';*/
						/*html += '	<p id="uploadFailure_'+i+'" class="file_failure">上传失败，请重试</p>';*/
						html += '	<p id="uploadSuccess_v_'+i+'" class="file_success" style="display:inline-block;"></p>';
						html += '</div>';
					}
					if(para.acitionType == "view"){//查看图片
						$("#preview").html(html);
					}else if(para.acitionType == "update"){//修改图片
						$(".add_upload").before(html);
						$("#"+param.inputId).val(imgUrl);
						if(para.multiple){
						...
						$(".file_del1").click(function() {
							...
							var ele = $(this).parent().parent().parent();//一个图片整个区域
							var ele_url = ele.find(".upload_image").attr("src");
							$.each( urlArr, function( index, u_ele ) {
									if(u_ele == ele_url){
							...
							ele.remove();
							var url = "";
							$.each( urlArr, function( index, ele ) {
									url +=ele +",";
								}
							)
							url = url.substring(0, url.length-1);
							$("#"+param.inputId).val(url);
						});
			...
			this.createCorePlug = function(){
				var params = {
					...
					uploadInput: para.uploadInput.get(0),//$("#fileSubmit").get(0),
					...
					onSuccess: function(file, response) {
						...
						var url = "";
						$("#uploadImage_"+file.index).attr("src",data.url);
			...
			this.addEvent = function(){
				...
				// 绑定上传点击事件
				para.uploadInput.bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(ZYFILE.funReturnNeedFiles().length > 0){
						para.uploadInput.click();
					}else{
						if(para.needFlage=="1"){
							para.onComplete(response);
							//para.uploadInput.click();
				...
				if($("#rapidAddImg").length > 0){
					...
					$("#rapidAddImg").bind("click", function(e){
						var val = param.imgUrl != undefined ? param.imgUrl :'';//修改时图片返回的结果
						var fileVal = $("#fileImage").val() != "" ? $("#fileImage").val():val;
						if(para.multiple){
							$("#fileImage").click();
						}else if(fileVal == ""){
							$("#fileImage").click();
